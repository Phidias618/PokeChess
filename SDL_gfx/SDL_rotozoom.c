...
#define SDL_SRCCOLORKEY 0b00001000
...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* src, int numClockwiseTurns) 
{
	...
	if (!src || SDL_BITSPERPIXEL(src->format) != 32) { return NULL; }
	...
	bpp = SDL_BYTESPERPIXEL(src->format);
...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	...
	*/
	...
	if ((is32bit) || (SDL_BITSPERPIXEL(src->format) == 8)) {
		...
		*/
		rz_src = SDL_CreateSurface(src->w, src->h, src->format);
		...
		*/
		if (is32bit) {
			...
			*/
			...
			SDL_SetSurfaceColorKey(rz_dst, SDL_SRCCOLORKEY, _colorkey(rz_src));
		} else {
			/*
			* Copy palette and colorkey info 
			*/
			SDL_Palette* p1 = SDL_GetSurfacePalette(rz_src);
			SDL_Palette* p2 = SDL_GetSurfacePalette(rz_dst);
			for (i = 0; i < (p1->ncolors); i++) {
				p2->colors[i] = p1->colors[i];
			}
			p2->ncolors = p1->ncolors;
			/*
...
SDL_Surface *zoomSurface(SDL_Surface * src, double zoomx, double zoomy, int smooth)
{
	...
	*/
	if (is32bit) {
		...
		*/
		SDL_Palette* p1 = SDL_GetSurfacePalette(rz_src);
		SDL_SetSurfacePalette(rz_dst, p1);
		/*
		* Call the 8bit transformation routine to do the zooming 
		*/
		_zoomSurfaceY(rz_src, rz_dst, flipx, flipy);
...
SDL_Surface *shrinkSurface(SDL_Surface *src, int factorx, int factory)
{
	...
	*/
	if (is32bit==1) {
		...
		*/
		result = SDL_SetSurfaceColorKey(rz_dst, SDL_SRCCOLORKEY, _colorkey(rz_src));
		if (result!=0) {
			haveError = 1;
			goto exitShrinkSurface;
		}		
