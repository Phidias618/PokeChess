...
public:
	...
	virtual void change_holder(Piece* new_holder) {
		holder = new_holder;
...
public:
	...
	static int usefulness_tier(Piece* piece) {
		effectiveness const e = typechart[type][piece->type];
...
public:
	...
	virtual void evasion_modifier(move_data& data) {
		data.miss_rate = 1 - ((1 - data.miss_rate) * 0.9);
	}
...
public:
	...
	virtual void revenge(move_data& data) {
		if (holder->is_in_graveyard) {
			...
			holder->set_item(NULL, false);

			data.attacker->set_item(this);
		}
	}
...
public:
	...
	virtual void adjust_reachable() {
		holder->reachable_square &= holder->board.presence_mask;
	}
...
public:
	...
	virtual void adjust_reachable() {
		Uint64 const old = holder->reachable_square;
		Knight knight = *dynacast_unsafe<Knight>(holder);
		knight.square = holder->square;
		knight.set_reachable();
		knight.reachable_square &= ~holder->board.presence_mask;
		holder->item_reachable_square = knight.reachable_square & ~old;
		holder->reachable_square = knight.reachable_square | old;
	}
...
public:
	...
	static int usefulness_tier(Piece* piece) {
		return (game.with_ability and piece->ability_no_guard) ? 0 : 2;
	}
...
public:
	...
	void change_holder(Piece* new_holder) {
	...
	static int usefulness_tier(Piece* piece) {
		return (piece->type != typeless and typechart[bug][piece->type] <= not_very_effective) ? 2 : 1;
	}
...
public:
	...
	virtual void adjust_reachable() { 
		...
		Uint64 old_mask = holder->reachable_square;
		holder->set_reachable();
		holder->item_reachable_square = holder->item_reachable_square& ~old_mask;
...
public:
	...
	void promote(Piece* new_holder) {
		new_holder->set_item(this, false);
...
public:
	...
	virtual void adjust_reachable() {
		if (holder->Class == Pawn::cls) {
			...
			if (0 <= 0 and Y <= 7) {
				holder->reachable_square &= ~(0b1111'1111ull << (Y << 3));
			}
		}
		else if (holder->Class == King::cls) {
			if (not holder->has_already_move) {
	...
	virtual bool is_move_disallowed(Square& target) {
		if (holder->Class == Pawn::cls) {
			Pawn* piece = dynacast<Pawn>(holder);
			...
			King* piece = dynacast<King>(holder);
